...
- Both allocations are currently sent to the same immutable admin wallet, but the contracts expose constants and emit per-swap accounting events so the split can be tracked off-chain.
...
- `ProtocolFeePaid(sender, token, recipient, totalAmount, rewardAmount, developmentAmount)`
